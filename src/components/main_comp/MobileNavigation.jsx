    ...
    return (
        <motion.div variants={mobileMenuContainer} initial="initial" animate="animate" exit="exit" className="mobile-menu d-lg-none d-flex flex-column px-4 px-sm-5 gap-5 pt-5">
            <motion.div variants={titleAndBack} initial="initial" animate="animate" exit="exit" className='d-flex align-items-center justify-content-between'>
                ...
                }} tabIndex="0" className='portfolio-logo w-50' src="/images/Logo.png" alt="Adrian Marcelo portfolio logo" loading='lazy' />
                <div role="button" id="hamburger-icon" onClick={() => setMobileMenuOpen(false)}>
                    <FaArrowRight style={{ width: '2rem', height: '3rem' }} className='text-white' />
                </div>
            </motion.div>

            {
                items.map(item => (
                    <motion.span role='button' key={item.title} tabIndex='0' onClick={() => {

                        document.querySelector(item.scrollSpy).scrollIntoView({ behavior: 'smooth' });
                        setMobileMenuOpen(false);
                    }} variants={mobileMenu} className='display-4 text-contrastColor'>{item.title}</motion.span>
                ))
            }
